...
class Scrap
  ...
  def get_house_details
    @browser.search("td.cuerporesultado>table").slice(1..-2).each do |table|
      begin
      price = table.search("td.tituloresult>a").text.gsub(/[,|$ ]/, "")
      neighborhood = table.search("td.tituloresult>h2").slice(2..-1).text.strip
      city = table.search("td.tituloresult>h2").slice(1..-2).text.gsub(/,/,"").strip
      ...
      description = "colonia: "+neighborhood+" ciudad: "+city+" pisos: "+floors+" area: "+area
      house = House.new(:price => price, :user_id => 1, :latitude => latitude, :longitude => longitude, :description => description )
      house.save
